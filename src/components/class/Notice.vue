  ...
  <div>
    ...
    <div class="content">
      <div class="content_topRight">
        <span class="span_text" @click="courseDialogVisible = true" >+发布公告</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getCourseNotice () {
      await this.$http.get('http://localhost:8080/notice/getCourseNotice/' + (this.queryInfo.pageNum - 1) + '/' + this.queryInfo.pageSize + '/' + window.sessionStorage.getItem('loginId') + '').then((res) => {
        console.log(res)
        this.courseList = res.data.content
