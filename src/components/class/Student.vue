      ...
      <div class="left_content">
        <el-menu default-active="1" class="el-menu" :collapse="isCollapse" v-model="editForm">
          <div class="user">
            ...
            <router-link to="/studentUserManager">
              <el-menu-item index="1" class="account">
                <span class="manageBtn">账号管理</span>
              </el-menu-item>
            </router-link>
          </div>
          <router-link to="/studentClassManager">
...
export default {
  ...
  methods: {
    async editHead () {
      ...
      const { data: res } = await this.$http.get('http://localhost:8080/student/changeHeadById/' + window.sessionStorage.getItem('loginId') + '/' + this.imageURL + '')
      console.log(res)
      if (res.code !== 200) {
    ...
    async showEditDialog () {
      const { data: res } = await this.$http.get('http://localhost:8080/student/findStudentById/' + window.sessionStorage.getItem('loginId') + '')
      console.log(res)
      this.editForm = res
    },
    editStu () {
      this.$refs.editFormRef.validate(async valid => {
        console.log(valid)
        if (!valid) return
        const { data: res } = await this.$http.post('http://localhost:8080/student/consummateStudentById/' + window.sessionStorage.getItem('loginId'), this.editForm)
        console.log(res)
        if (res.code !== 200) {
    ...
    async checkInfo () {
      await this.$http.get('http://localhost:8080/student/checkInfo/student/' + window.sessionStorage.getItem('loginId') + '').then((res) => {
        console.log(res)
