        ...
        <div class="row">
          ...
          <div class="mod-search js-common-search">
            <div class="mod-course-list" >
              <div class="resdetail-bd js-dot-resList" id="dot_content_resList" v-for="(item, key) in resultList" :key="key" :item="item">
...
export default {
  ...
  created () {
    this.findBooksByKeyWords()
  },
  methods: {
    async findBooksByKeyWords () {
      await this.$http.get('http://localhost:8080/book/findBooksByKeyWords/' + (this.queryInfo.pageNum - 1) + '/' + this.queryInfo.pageSize + '/' + window.sessionStorage.getItem('bookKeysWords') + '').then((res) => {
        console.log(res)
